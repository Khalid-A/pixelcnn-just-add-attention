...
from models.attentionConv2d import AttentionConv2d

# import os
# os.environ['KMP_DUPLICATE_LIB_OK']='True'

class ConditionalAttCNNpp(ConditionedGenerativeModel):
    def __init__(self, embd_size, img_shape, nr_resnet=5, nr_filters=80, nr_logistic_mix=10):
        super(ConditionalAttCNNpp, self).__init__(embd_size)
        self.pixel_cnn_model = AttCNNpp(nr_resnet=nr_resnet, nr_filters=nr_filters, nr_logistic_mix=nr_logistic_mix,
...
class AttCNNpp(nn.Module):
    def __init__(self, nr_resnet=5, nr_filters=80, nr_logistic_mix=10, embedding_size=768, input_channels=3):
        ...
        self.u_init = down_shifted_conv2d(input_channels + 1, nr_filters, filter_size=(2, 3), 
                                          shift_output_down=True, augment=True)
    ...
    def forward(self, x, h, sample=False):
        ...
        x_out[torch.isnan(x_out)] = 0 ## REPLACE NAN WITH 0
        return x_out
...
class PixelCNNLayer_down(nn.Module):
    ...
    def forward(self, u, ul, u_list, ul_list, cond_embedding):
        ...
        u[torch.isnan(u)] = 0 ## REPLACE NAN WITH 0
        ul[torch.isnan(ul)] = 0 ## REPLACE NAN WITH 0
        return u, ul
...
class down_shifted_conv2d(nn.Module):
    def __init__(self, num_filters_in, num_filters_out, filter_size=(2, 3), stride=(1, 1),
                 shift_output_down=False, augment=False, norm='weight_norm'):
    ...
    def forward(self, x):
        x_pad = self.pad(x)
        x = self.conv(x, x_pad) if self.augmented else self.conv(x_pad)
        x = self.bn(x) if self.norm == 'batch_norm' else x
        # return self.down_shift(x) if self.shift_output_down else x

        output = self.down_shift(x) if self.shift_output_down else x
        output[torch.isnan(output)] = 0 ## REPLACE NAN WITH 0
        return output 
...
class down_shifted_deconv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        output = x[:, :, :(xs[2] - self.filter_size[0] + 1),
                 int((self.filter_size[1] - 1) / 2):(xs[3] - int((self.filter_size[1] - 1) / 2))]
        output[torch.isnan(output)] = 0 ## REPLACE NAN WITH 0
        return output 
...
class down_right_shifted_conv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        output = self.right_shift(x) if self.shift_output_right else x
        output[torch.isnan(output)] = 0 ## REPLACE NAN WITH 0
        return output 
...
class down_right_shifted_deconv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        x[torch.isnan(x)] = 0 ## REPLACE NAN WITH 0
        return x
