...
class ConditionalAttPixelCNNpp(ConditionedGenerativeModel):
    def __init__(self, embd_size, img_shape, nr_resnet=5, nr_filters=80, nr_logistic_mix=10):
        super(ConditionalAttPixelCNNpp, self).__init__(embd_size)
...
class AttCNNpp(nn.Module):
    def __init__(self, nr_resnet=5, nr_filters=80, nr_logistic_mix=10, embedding_size=768, input_channels=3):
        ...
        self.u_init = down_shifted_conv2d(input_channels + 1, nr_filters, filter_size=(2, 3), 
                                          shift_output_down=True, attention=True)
        ...
        num_mix = 10
        self.nin_out = nin(nr_filters, num_mix * nr_logistic_mix, attention=False)
...
class nin(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.add_attention:
            ...
            x_pos = self.pos(x.view(b, l, h * w))
            x_att, _ = self.attn(x_pos, x_pos, x_pos)
            x = x_att.view(b, l, h, w)
